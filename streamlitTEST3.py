...
df2 = pd.DataFrame(np.random.rand(200,2)/[50,50]+[35.69,139.70],columns=['lat','lon'])
st.dataframe(df2.style.highlight_max(axis=0))
st.map(df2)

"""
### 以下に機械学習モデルで学習
## Gurobi Machine Learningで最適化
...
# Read data
data = pd.read_csv("C:/Users/ttake/Documents/MBS_Lessons/MBS2022/DataSet/CRMMaster20230114.csv",encoding="utf_8")
data.dtypes
...
data["total_spent"]=data["total_spent"].str.replace(',', '').astype(int)
# data.dtypes
data
